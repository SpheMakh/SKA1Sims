...
format = "%(lo0)s %(lores)s %(lofreq)d %(freq)d %(dec)d %(resbin)s %(ridx)d %(weight)s %(widx)d " + \
  "%(noise166).2g %(flux10).2f %(snr10).2f %(hours).2f %(snravg).2f %(noise50).2g %(psf_area).2g %(psf_sym).2f %(psf_mean).4f %(noise50k).2g %(speed).2g %(speed_avg).2g\n";
...
for kk,(noise50,origkey) in PIXNOISE.items():
  ...
  snr1 = numpy.zeros(4,float)
  speed_freq = numpy.zeros(4,float)
  for i,freq1 in enumerate((650,800,1000,1400)):
